...
int checkresult(S_BOARD *pos) {
    ...
    if (pos->fiftyMove > 100) {
     printf("1/2-1/2 {fifty move rule (claimed by seeChess)}\n"); return TRUE;
    }

    if (ThreeFoldRep(pos) >= 2) {
     printf("1/2-1/2 {3-fold repetition (claimed by seeChess)}\n"); return TRUE;
    }

	if (DrawMaterial(pos) == TRUE) {
     printf("1/2-1/2 {insufficient material (claimed by seeChess)}\n"); return TRUE;
    }
	...
	if(InCheck == TRUE)	{
	    if(pos->side == WHITE) {
	      printf("0-1 {black mates (claimed by seeChess)}\n");return TRUE;
        } else {
	      printf("0-1 {white mates (claimed by seeChess)}\n");return TRUE;
        }
    } else {
      printf("\n1/2-1/2 {stalemate (claimed by seeChess)}\n");return TRUE;
    }
	return FALSE;
}
